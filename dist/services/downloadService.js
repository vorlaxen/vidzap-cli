...
const sanitizeFilename = (filename) => filename.replace(/[\x00-\x1f<>:"/\\|?*\u{7f}]/gu, "-").trim();
const ensureDirExists = (dir) => {
    if (!fs.existsSync(dir))
        fs.mkdirSync(dir, { recursive: true });
};
const getAvailableFilename = (filePath) => {
...
const findClosestResolution = (formats, targetRes) => {
...
export const fetchVideoMetadata = async (url) => {
    ...
    if (info.videoDetails.isLiveContent)
        throw new Error("Live stream not supported.");
    videoCache.set(url, info);
    return info;
};
export const getAvailableResolutions = (formats) => {
    return formats
...
export const downloadService = async (options) => {
    const { url, container = "mp4", quality = "720p", savePath, mp3Bitrate = 128 } = options;
    try {
